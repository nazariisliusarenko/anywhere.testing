...
function render_toggles_block($attributes) {
    ...
    $output .= '<section class="toggles">';
    
      $output .= '<div class="row"><div class="col-md-12 center ac">';
      $output .= '<h2>'.$attributes["heading"].'</h2>';
      $output .= '<div>'. $attributes["textareaDescription"] .'</div>';
      $output .= '</div></div>';

      $output .='<div class="container"><div class="row"><div class="accordion">';
      foreach($recent_posts as $recent) {
    	$image = wp_get_attachment_image_src(get_post_thumbnail_id($recent["ID"]), 'single-post-thumbnail' );
          ...
          <div class="content">
            '.strip_tags($recent["post_content"],"<p>,<br/>,<ul>,<li>").'
          </div>
          </div>';
          $output .= '</div>';
	    }  
