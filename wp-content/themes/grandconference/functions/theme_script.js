...
{	
	...
	jQuery("#content_builder_sort li a.ppb_duplicate").on( 'click', function(){
		...
		//Display edit button
	    var editURL = tgAjax.ajaxurl+'?action=grandconference_ppb&ppb_post_type='+postType+'&shortcode='+myCheckId+'&rel='+randomId+'&width=800&height=900&page_id='+jQuery('#ppb_page_id').val();
...
jQuery(document).ready(function(){
	...
	jQuery("#ppb_module_wrapper li").on( 'click', function(){
	    ...
	    if(myCheckId != '' && targetType == 'module')
	    {
			...
			//Display edit button
	    	var editURL = tgAjax.ajaxurl+'?action=grandconference_ppb&ppb_post_type='+postType+'&shortcode='+myCheckId+'&rel='+randomId+'&width=800&height=900&page_id='+jQuery('#ppb_page_id').val()+'&builder_action=add';
	    	builderItem+= '<a data-rel="'+randomId+'" href="'+editURL+'" class="ppb_edit"><i class="fa fa-edit"></i></a>';
	...
	jQuery('#import_demo_content li').on( 'click', function(){
	    ...
	    var selectedDemo = jQuery(this).data('demo');
	    jQuery('#grandconference_import_demo_content').val(selectedDemo);
	});
	...
	jQuery('.pp_import_content_button').on( 'click', function(){
        ...
        var data = {
            'action': 'grandconference_import_demo_content',
            'demo': selectDemo
        };
	...
	//Custom functions for handle ticket options box
	var bookingMethod = jQuery('#ticket_booking_method').val();
	switch(bookingMethod) 
	{
	    case 'woocommerce_product':
	        jQuery('#post_option_ticket_booking_product').show();
	        jQuery('#post_option_ticket_booking_url').hide();
	    break;
	    
	    case 'external':
	        jQuery('#post_option_ticket_booking_product').hide();
	        jQuery('#post_option_ticket_booking_url').show();
	    break;
	}
	
	jQuery('#ticket_booking_method').on( 'change', function(){
    ...
    jQuery('.pp_get_styling_button').on( 'click', function(){
        ...
        var data = {
            'action': 'grandconference_get_styling',
