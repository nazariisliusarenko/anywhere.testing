...
function grandconference_posts($sort = 'recent', $items = 3, $echo = TRUE, $show_thumb = TRUE) 
{
	...
	if($sort == 'recent')
	{
		...
		echo "<pre>" . "here"; print_r($posts); exit;
		$title = esc_html__('Recent Posts', 'grandconference');
	}
	
	if(!empty($posts))
	{
		$return_html.= '<h2 class="widgettitle w-mg"><span>'.$title.'</span></h2>';
...
function grandconference_cat_posts($cat_id = '', $items = 5, $echo = TRUE, $show_thumb = TRUE) 
{
	...
	$count_post = count($posts);
        $category_url = "https://stepconference.com/category/".$category_link;
	
	if(!empty($posts))
	{

		$return_html.= '<h2 class="widgettitle w-mg"><a href="'.$category_link.'" style="color:#000 !important;"><span>'.$title.'</span></a></h2>';
		$return_html.= '<ul class="posts blog ';
		...
		$return_html.= '">';
			...
			foreach($posts as $key => $post)
			{
				...
				if(!empty($show_thumb) && has_post_thumbnail($post->ID, 'thumbnail'))
				{
					...
					$return_html.= '<div class="post_circle_thumb col-md-3"><a href="'.get_permalink($post->ID).'"><img class="alignleft frame post_thumb" src="'.$image_url[0].'" alt="" /></a></div>';
				}
				
				$return_html.= '<div class="col-md-9"><a href="'.get_permalink($post->ID).'">'.grandconference_substr($post->post_title, 50).'</a><div class="post_attribute">'.date_i18n(GRANDCONFERENCE_THEMEDATEFORMAT, get_the_time('U', $post->ID)).'</div></div>';
				$return_html.= '<div style="clear: both"></div></li>';
			}	
...
function grandconference_get_flickr($settings) 
{
	...
	$flickr_cache_path = GRANDCONFERENCE_THEMEUPLOAD.'/flickr_'.$settings['id'].'_'.$settings['items'].'.cache';
...
{   
    ...
    if(!empty($username) && !empty($access_token))
    {
		...
		if(!file_exists($instagram_cache_path) OR $instagram_cache_timer > 15)
		{
		    ...
		    {
		    	$shots = $isg->getTagMedia(array('count'=> $items), GRANDCONFERENCE_THEMEDEMOIG); 
		    }
...
function grandconference_hex_darker($hex,$factor = 30)
{
    $new_hex = '';
...
function grandconference_get_demo_url($path = '')
{
	$demo_url = 'http://themes.themegoods.com/grandconference/demo'.$path;
	return $demo_url;
}
